...
export function createBookingStore(){
 ...
 return {
  ...
  create({mentorId,diagnosis,date,time,duration,note,consent}){
   ...
   if(!diagnosis?.goal||!diagnosis?.level||!diagnosis?.knowledge||!diagnosis?.id)throw new Error('진단서의 목표와 사전 지식을 채워주세요.');
   ...
   requests.push(r);message(r,'system','매칭 신청이 접수되었습니다. 진단서가 이 채팅방에 첨부되었어요.');message(r,'diagnosis','',{diagnosis:snapshot,role:'learner'});
   if(r.note)message(r,'text',r.note,{role:'learner'});
   return r;
  },
  acknowledge(requestId,actor){role(actor,'mentor');const r=get(requestId);active(r);if(r.status===0){r.status=1;const m=mentors.find(x=>x.id===r.mentorId),d=r.diagnosis;message(r,'system','강사가 신청을 수락했어요. 이제 대화를 나눌 수 있어요.');message(r,'text',`안녕하세요, ${m.name}입니다. 진단서 잘 봤어요.`+`\n${d.category}를 ${d.knowledge==='아예 몰라요'?'처음부터':`(${d.knowledge}) 상태에서`} 시작하시는군요.`+`\n\n편하게 궁금한 점부터 물어봐 주세요. 일정과 범위는 여기서 같이 맞춰봐요.`,{role:'mentor'});}return r;},
  send(requestId,actor,text){if(!['learner','mentor'].includes(actor))throw new Error('화면 역할을 선택해주세요.');if(actor==='learner'&&get(requestId).status===0)throw new Error('강사가 신청을 수락하면 대화를 시작할 수 있어요.');text=String(text||'').trim();if(!text||text.length>1600)throw new Error('메시지는 1~1600자로 입력해주세요.');const r=get(requestId);if(actor==='mentor'&&r.status===0){r.status=1;message(r,'system','상담이 시작되었어요. 일정과 수업 범위를 조율해주세요.');}message(r,'text',text,{role:actor});return r;},
  requestQuote(requestId,actor){role(actor,'learner');const r=get(requestId);active(r);if(r.status===0)throw new Error('강사가 신청을 수락한 뒤에 요청할 수 있어요.');if(r.reservation)throw new Error('이미 예약이 확정되었습니다.');message(r,'text','진단서를 바탕으로 수업 범위와 가능한 일정, 견적을 보내주실 수 있을까요?',{role:'learner'});return r;},
  proposeSchedule(requestId,actor,input){const r=get(requestId);active(r);if(actor==='learner'&&r.status===0)throw new Error('강사가 신청을 수락한 뒤에 일정을 제안할 수 있어요.');if(r.reservation)throw new Error('확정된 예약의 일정은 변경할 수 없어요.');if(!['learner','mentor'].includes(actor))throw new Error('화면 역할을 선택해주세요.');const slot=scheduleValid(input.date,input.time,input.duration);message(r,'schedule','이 일정으로 진행할 수 있을까요?',{role:actor,slot});r.preferred=slot;if(actor==='mentor'&&r.status===0)r.status=1;return r;},
