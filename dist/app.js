...
const catMark=(size=26)=>`<svg width="${size}" height="${size}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M4 12.4V5.3c0-.85.93-1.37 1.65-.92L9.9 7.05a8.4 8.4 0 0 1 4.2 0l4.25-2.67c.72-.45 1.65.07 1.65.92v7.1a8 8 0 0 1-16 0Z"/><circle cx="9.3" cy="12.2" r="1.05" fill="currentColor" stroke="none"/><circle cx="14.7" cy="12.2" r="1.05" fill="currentColor" stroke="none"/><path d="M12 14.5l-.85.8h1.7z" fill="currentColor" stroke="none"/><path d="M12 15.3v1.05M12 16.35c-.55.6-1.5.6-2.05.05M12 16.35c.55.6 1.5.6 2.05.05"/></svg>`;
const brand=()=>`<a class="brand" href="#/" aria-label="먀칭 홈"><span class="brand-mark">${catMark(26)}</span><span>먀칭</span></a>`;
const header=(active='home',extra='')=>`<header class="site-header ${extra}">${brand()}<nav class="nav" aria-label="메인 메뉴"><a class="${active==='home'?'active':''}" href="#/">홈</a><a class="${active==='mentors'?'active':''}" href="#/mentors">강사 둘러보기</a><a class="${active==='chat'?'active':''}" href="#/chat">AI 상담</a><a class="${active==='messages'?'active':''}" href="#/messages">채팅·예약</a></nav><div class="header-actions"><a class="teach-link ${active==='teach'?'active':''}" href="#/teach">강사로 등록</a><a class="teach-link admin-link ${active==='admin'?'active':''}" href="#/admin">운영 콘솔</a><span class="demo-tag">UI PREVIEW</span><a class="btn btn-dark" href="#/chat">나에게 맞는 강사 찾기 ${icon('arrow',15)}</a></div></header>`;
const footer=()=>`<footer class="site-footer"><b>먀칭</b><span>서비스 UI 체험판 · 강사 사진과 이력은 가상 예시입니다. 상담·메시지·견적·결제는 시연용이며 실제 전달이나 청구는 발생하지 않습니다.</span></footer>`;
...
function teachForm(pre=null){const p=k=>pre?(pre[k]||''):'';const car=i=>pre&&pre.career&&pre.career[i]?pre.career[i][1]:'';const out=i=>pre&&pre.outcomes?(pre.outcomes[i]||''):'';
 ...
 return `${header('teach')}<main id="main" class="teach-page"><div class="teach-head"><p class="section-kicker">먀칭에서 가르치기</p><h1>${pre?'프로필을<br>고쳐볼까요?':'가르치는 사람으로<br>등록해볼까요?'}</h1><p>${pre?'고친 내용은 저장하는 즉시 수강생에게 보이는 프로필에 반영됩니다.':'등록하면 운영자 검수를 거쳐 강사 목록에 올라갑니다. 수강생은 목표와 현재 수준이 정리된 진단서를 들고 찾아와요.'}</p></div><form id="teach-form" class="teach-form" novalidate>
...
function routeGrid(){return `<section class="section route-section"><div class="section-heading"><div><p class="section-kicker">WHERE TO START</p><h2>어디로 가볼까요?</h2><p>고르면 그 분야부터 상담이 시작돼요.</p></div></div><div class="route-grid">${routes.map((r,i)=>`<button class="route-tile" data-route="${i}" title="${r.desc}"><span class="route-tile-icon">${icon(r.icon,24)}</span><strong>${r.cat}</strong></button>`).join('')}</div></section>`;}
function faceCard(m){return `<a class="face" href="#/mentor/${m.id}" tabindex="-1" aria-hidden="true"><div class="face-img">${portrait(m)}</div><div class="face-meta"><b>${escape(m.name)}</b><small>${escape(m.role)} · ${escape(m.years)}</small></div></a>`;}
function heroMarquee(){const half=`<div class="faces-half">${[0,1].map(()=>mentors.map(faceCard).join('')).join('')}</div>`;return `<div class="faces"><div class="faces-track">${half}${half}</div><p class="sr-only">등록된 강사 ${mentors.map(m=>m.name+' '+m.role).join(', ')}. 전체 목록은 강사 모두 보기에서 확인할 수 있습니다.</p></div>`;}
function home(){return `${header('home','site-header-float')}<main id="main"><section class="hero"><h1>AI, <em>어디서 막혔나요?</em></h1><p class="hero-note"><span>실무에서 AI로 <b>결과를 내는</b> 강사가 기다리고 있어요</span></p>${heroMarquee()}<div class="hero-cta"><a class="btn btn-dark" href="#/chat">내게 맞는 전문가 찾기 ${icon('arrow',15)}</a><a class="text-link" href="#/mentors">강사 모두 보기 ${icon('right',14)}</a></div></section><div class="container">${routeGrid()}<section class="section"><div class="section-heading"><div><p class="section-kicker">FIND YOUR GUIDE</p><h2>나의 다음 단계를 함께할 전문가</h2><p>무엇부터 할지 모를 때도, 만들다가 막혔을 때도.</p></div><div class="section-actions"><a class="text-link" href="#/mentors">모두 보기 ${icon('right',14)}</a>${arrows('home-shelf')}</div></div><div class="shelf" id="home-shelf">${[mentors[2],mentors[1],mentors[0],mentors[3],mentors[4],mentors[5]].map(card).join('')}</div></section><section class="teach-cta"><div><h2>AI를 가르치고 계신가요?</h2><p>내가 가장 잘 도울 수 있는 사람을 만나보세요. 목표와 현재 수준이 정리된 상태로 연결해드려요.</p></div><a class="btn btn-dark" href="#/teach">강사로 등록하기 ${icon('arrow',15)}</a></section></div></main>${footer()}`;}
let filter='전체';
...
let seenCount=0;
const TOOL_OPTIONS=[['아직 거의 안 써봤어요','처음 시작해요'],['ChatGPT에 가끔 물어보는 정도예요','처음 시작해요'],['업무에 종종 쓰고 있어요','도구를 써봤어요'],['여러 도구를 골라 쓰는 편이에요','도구를 써봤어요'],['자동화하거나 직접 만들어봤어요','직접 만들고 있어요']];
const KNOW_OPTIONS=['아예 몰라요','용어만 들어봤어요','기본 개념은 알아요','실무에 적용해본 적 있어요'];
const FORMAT_OPTIONS=['온라인으로 주말에','온라인으로 평일 저녁에','오프라인으로 만나서','아직 정하지 못했어요'];
const toolLabels=()=>TOOL_OPTIONS.map(o=>o[0]);
function toolLevel(t){const hit=TOOL_OPTIONS.find(([label])=>label===t);if(hit)return hit[1];
 if(/안\s?써|처음|없어|없음|몰라|모름|초보/.test(t))return '처음 시작해요';
 if(/만들|자동화|배포|개발|코딩|api/i.test(t))return '직접 만들고 있어요';
 if(/써|사용|해봤|가끔|종종|자주|경험/.test(t))return '도구를 써봤어요';
 return null;}
const Q1='안녕하세요, 먀칭이에요.\n네 가지만 여쭤볼게요.\n\n먼저, AI 도구는 어느 정도 써보셨어요?';
function newChat(mentorId=null){seenCount=0;chat={mentorId,goal:'',category:mentors.find(m=>m.id===mentorId)?.category||null,tools:'',level:'',knowledge:'',format:'',blocker:'',stage:'tools',messages:[]};bot(Q1,toolLabels());}
...
function advance(text){text=String(text).trim().slice(0,1200);if(!text)return;user(text);const m=mentors.find(m=>m.id===chat.mentorId);
 if(chat.stage==='tools'){
  const lv=toolLevel(text);
  if(!lv){bot('가장 가까운 것을 아래에서 골라주세요. 직접 적으셔도 괜찮아요.',toolLabels());}
  else{chat.tools=text;chat.level=lv;chat.stage='goal';bot('좋아요.\n그럼 무엇을 해보고 싶으세요?\n가장 가까운 분야를 골라주시거나, 직접 적어주셔도 좋아요.',categories.slice(1));}
 }else if(chat.stage==='goal'){
  const c=categories.slice(1).includes(text)?text:inferCategory(text);
  if(!c){bot('아래 여섯 가지 중 가장 가까운 분야를 골라주세요.',categories.slice(1));}
  else{chat.category=c;chat.goal=text;chat.stage='knowledge';bot(`${c}, 좋아요.\n이 분야 사전 지식은 어느 정도 되세요?`,KNOW_OPTIONS);}
 }else if(chat.stage==='knowledge'){
  chat.knowledge=text;chat.stage='format';
  bot(text==='아예 몰라요'?'괜찮아요. 모르는 상태에서 시작하는 분이 제일 많아요.\n마지막으로, 어떤 형식이 편하세요?':'마지막이에요.\n어떤 형식의 과외가 편하세요?',FORMAT_OPTIONS);
 }else if(chat.stage==='format'){
  chat.format=text;chat.stage='result';
  bot(m?`${m.name} 강사와 상담할 내용을 정리했어요.\n맞는지 확인해주세요.`:'말씀해주신 네 가지를 정리했어요.\n맞는지 확인하고, 나에게 맞는 강사를 보러 가요.',null,true);
 }else{
  chat.blocker=text;bot('추가로 알려주신 내용을 진단서에 반영했어요.\n분야나 수준을 바꾸고 싶다면 새 상담을 시작해주세요.',null,true);
 }
 renderChat(true);
 if(chat.stage==='result')matching.finishDiagnosis({...chat,steps:routeSteps()});
}
function routeSteps(){const m=mentors.find(x=>x.category===chat.category)||mentors[2];let steps=[...m.sessions];if(chat.level==='처음 시작해요'||chat.knowledge==='아예 몰라요')steps[0]='개념을 짚고 사용할 AI 고르기';if(/오류|에러/.test(chat.blocker))steps[1]='현재 작업과 오류를 전문가와 함께 점검하기';return steps;}
function result(){return `<div class="result-box"><div class="result-heading"><div class="eyebrow">${icon('check',15)} 이렇게 이해했어요</div><h2>맞는지 한 번만 확인해주세요</h2></div><dl class="result-check"><div><dt>하고 싶은 것</dt><dd>${escape(chat.goal)}</dd></div><div><dt>AI 도구 경험</dt><dd>${escape(chat.tools||chat.level)}</dd></div><div><dt>원하는 형식</dt><dd>${escape(chat.format||'아직 정하지 못함')}</dd></div><div><dt>사전 지식</dt><dd>${escape(chat.knowledge||'—')}</dd></div></dl><p class="result-memo">다르게 이해한 부분이 있으면 아래에 적어주세요. 바로 고쳐서 반영할게요.</p></div><div class="result-actions"><a class="btn btn-dark btn-wide" href="#/finding">나에게 꼭 맞는 튜터 찾으러 가기 ${icon('arrow',16)}</a></div>`;}
...
function chatHTML(){const progress={tools:0,goal:1,knowledge:2,format:2,result:3}[chat.stage];return `<div class="chat-layout"><aside class="chat-sidebar">${brand()}<button class="btn chat-new" data-reset>${icon('plus',17)} 새 상담 시작하기</button><p class="side-label">나를 위한 AI 활용</p><a class="side-link active" href="#/chat">${icon('chat',17)} 나의 AI 네비게이션</a><a class="side-link" href="#/mentors">${icon('grid',17)} 강사 둘러보기</a><a class="side-link" href="#/recommendations">${icon('spark',17)} 나의 추천 결과</a><a class="side-link" href="#/messages">${icon('chat',17)} 채팅·예약</a><a class="side-link" href="#/">${icon('route',17)} 홈으로 돌아가기</a><div class="side-plan">${icon('spark',22)}<strong>정답보다, 나에게 맞는 다음 단계.</strong><p>현재 수준과 하고 싶은 일을 알려주세요. 나에게 필요한 도움을 함께 찾아볼게요.</p></div><p class="side-demo">시나리오 상담 체험 · 실제 AI 연결 전</p></aside><main class="chat-main" id="main"><header class="chat-header"><div><a href="#/" class="mobile-brand" aria-label="홈으로">${icon('route',24)}</a><strong>나의 AI 네비게이션</strong><span class="demo-tag">체험</span></div><div><span class="desktop-label">경험 → 목표 → 형식 → 추천</span><div class="progress-pills" aria-label="상담 ${progress}단계 완료">${[0,1,2].map(i=>`<i class="${i<progress?'on':''}"></i>`).join('')}</div><a href="#/mentors">강사 보기 ${icon('right',15)}</a></div></header><div class="chat-body" id="messages" aria-live="polite" aria-relevant="additions text">${chat.messages.length?chat.messages.map((m,i)=>m.role==='user'?`<div class="chat-msg user${i>=seenCount?' is-new':''}"><div class="user-bubble">${escape(m.text)}</div></div>`:`<div class="chat-msg${i>=seenCount?' is-new':''}"><div class="bot-name"><span class="mini-mark">${icon('route',18)}</span>먀칭</div><div class="bot-bubble">${escape(m.text)}</div>${m.choices&&i===chat.messages.length-1?`<div class="answer-choices">${m.choices.map(c=>`<button class="choice" data-answer="${escape(c)}">${escape(c)}</button>`).join('')}</div>`:''}${m.result&&i===chat.messages.length-1?result():''}</div>`).join(''):welcome()}</div><div class="chat-input-wrap">${composer('chat-form',chat.stage==='result'?'상담에 추가할 내용을 남겨주세요.':'하고 싶은 일이나 지금 막힌 부분을 알려주세요.')}<p class="chat-input-note">시나리오 기반 상담 체험입니다. 개인정보나 업무 기밀은 입력하지 마세요.</p></div></main></div>`;}
...
let onHomeScroll=null;
function bindHome(){
 if(onHomeScroll){window.removeEventListener('scroll',onHomeScroll);onHomeScroll=null;}
 const h=document.querySelector('.site-header-float');if(!h)return;
 const band=document.querySelector('.faces');
 const fn=()=>{const t=Math.max(120,(band&&band.offsetHeight||300)*0.62);
  if((window.scrollY||0)>t)h.classList.add('is-on');else h.classList.remove('is-on');};
 onHomeScroll=fn;window.addEventListener('scroll',fn,{passive:true});fn();
}
function render(){const {path,query}=route();if(matching.renderRoute(path,query)){registerWebTools();return;}if(path==='/chat'){const mentorId=query.get('mentor');if(mentorId&&!mentors.some(m=>m.id===mentorId)){app.innerHTML=profile('missing');return;}if(!chat||chat.mentorId!==mentorId)newChat(mentorId);if(requestedGoal){const goal=requestedGoal;requestedGoal='';advance(goal);}else renderChat();document.title='AI 상담 · 먀칭';}
 else if(path==='/admin'){app.innerHTML=admin(query.get('tab'));document.title='운영 콘솔 · 먀칭';window.scrollTo(0,0);}else if(path==='/teach'){editing=false;pendingPhoto=null;app.innerHTML=teachForm();document.title='강사로 등록 · 먀칭';window.scrollTo(0,0);bindTeach();}else if(path==='/teach/edit'){if(!mySubmission){location.hash='#/teach';return;}editing=true;pendingPhoto=mySubmission.m.photo||null;app.innerHTML=teachForm(mySubmission.m);document.title='프로필 수정 · 먀칭';window.scrollTo(0,0);bindTeach();}else if(path==='/teach/done'){app.innerHTML=teachDone();document.title='등록 신청 완료 · 먀칭';window.scrollTo(0,0);bindTeach();}else if(path==='/teach/preview'){app.innerHTML=mySubmission?profile(mySubmission.m.id):teachForm();document.title='프로필 미리보기 · 먀칭';window.scrollTo(0,0);}else{if(path==='/mentors'){filter=categories.includes(query.get('category'))?query.get('category'):'전체';app.innerHTML=catalog();document.title='나에게 맞는 강사 찾기 · 먀칭';}else if(path.startsWith('/mentor/')){const id=path.split('/')[2];app.innerHTML=profile(id);document.title=(mentorById(id)?.name||'강사 상세')+' · 먀칭';}else{app.innerHTML=home();document.title='먀칭 — 나에게 맞는 AI 활용의 시작';}window.scrollTo(0,0);bindForms();bindHome();}registerWebTools();}
